...
Сколько раз пытаться выполнить `docker push` (и `docker buildx ... --push`) при ошибке. Перед каждой попыткой action делает `GET https://<registry>/v2/`, чтобы в логе было видно, доступен ли registry. Недоступность registry не прерывает ретраи — пуш всё равно будет выполнен. Интервал между попытками растёт экспоненциально: 2с, 4с, 8с, …, максимум 60с.
