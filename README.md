...
Тег образов, используемый вместо `tag`, когда сборка выполняется для git-тега (а не ветки). Если параметр пустой — используется `tag`. Поддерживает ту же шаблонизацию, что и [`tag`](#tag).

Пример: `tag-on-git-tag: '{{ ref }}'`
...
Сколько раз пытаться выполнить `docker push` (и `docker buildx ... --push`) при ошибке. Перед каждой попыткой action делает `GET https://<registry>/v2/`, чтобы в логе было видно, доступен ли registry. Недоступность registry не прерывает ретраи — пуш всё равно будет выполнен. Интервал между попытками — exponential backoff с full jitter: случайная пауза от 0 до 2с, 4с, 8с, …, максимум 60с.
